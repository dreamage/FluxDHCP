...
export class PoolManager {
  ...
  allocateIP(mac: string, requestedIP?: string, clientNetwork?: string): { ip: string; pool: Pool } | null {
    ...
    if (honorRequested && requestedIP && requestedIP !== '0.0.0.0') {
  ...
  /**
   * 在指定地址池中查找第一个可用 IP
   * 支持顺序分配（sequential）和随机分配（random）
   */
  findAvailableIPInPool(pool: Pool): string | null {
    ...
    let order = 'sequential';
    try {
      const row = this.db.prepare("SELECT value FROM config WHERE key = 'ip_allocation_order'").get() as { value: string } | undefined;
      if (row) order = row.value;
    } catch { /* ignore */ }
    ...
    if (order === 'random') {
      ...
      for (const offset of offsets) {
        const ip = this.numToIP(startNum + offset);
        if (!occupiedIPs.has(ip)) return ip;
      }
    } else {
      // 顺序分配：从头遍历
