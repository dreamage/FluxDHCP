...
/**
 * Format a datetime string from the database (UTC) to local time without milliseconds.
 */
export function formatLocalTimeNoMs(v: string | null | undefined): string {
  return formatLocalTime(v).replace(/\.\d{3}$/, '');
}
