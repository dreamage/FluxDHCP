...
- **IP 地址池管理** - 定义子网的起止 IP 范围、网关、DNS 服务器、子网掩码，支持按地址池设置租约时间，自动检测地址池重叠，新建/编辑时校验 IP 是否在子网范围内（前后端双重校验），原子化 IP 分配防止竞态条件。双视图可视化：颜色区分的 IP 地址块和含主机名/保留描述列的 IP 列表。
...
| **设置** | DHCP 服务控制、服务器配置、T1/T2 参数（带说明）、DHCP 日志保留天数、DECLINE 黑名单时长、配置导入导出（卡片式选择器、显示导出时间、含校验） |
